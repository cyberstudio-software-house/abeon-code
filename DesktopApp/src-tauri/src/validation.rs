...
/// Validate a session id that is used BOTH as a `claude` CLI argument and as a
/// `<id>.jsonl` filename stem. The allowlist `[A-Za-z0-9_-]` (no leading `-`,
/// bounded length, non-empty) makes the value safe on both surfaces at once:
/// it cannot contain shell metacharacters, whitespace, quotes, or a flag-style
/// `-` prefix (so a shell can't reinterpret it and argv can't be smuggled), and
/// it cannot contain `/` or `.` (so `join`-based path traversal is impossible).
/// This is the trust boundary for remote (mobile-originated) input in the bridge.
pub fn validate_session_id(id: &str) -> AppResult<()> {
    ...
    if id.starts_with('-') {
        return Err(AppError::InvalidInput("session id must not start with '-'".into()));
    }
...
pub fn validate_model(model: &str) -> AppResult<()> {
    if model.is_empty() || model.len() > MAX_MODEL_LEN {
        return Err(AppError::InvalidInput("model length out of range".into()));
    }
    if model.starts_with('-') {
        return Err(AppError::InvalidInput("model must not start with '-'".into()));
    }
    if !model
        .chars()
    ...
    {
        return Err(AppError::InvalidInput(
            "model contains invalid characters".into(),
        ));
    }
    Ok(())
}
